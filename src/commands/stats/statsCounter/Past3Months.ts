...
export async function past3Months(memberID: number, user: any): Promise<any> {
    try {
      const data: Record<string, number> = {};
      const today = dayjs();
      const pastDays = 90;
  
      const startDate = today.subtract(pastDays - 1, 'day').format('YYYY-MM-DD');
      ...
      const embed = new EmbedBuilder()
        .setColor('Random')
        .setTitle("📊 Past 3 Months' Activity Summary")
        .setAuthor({
